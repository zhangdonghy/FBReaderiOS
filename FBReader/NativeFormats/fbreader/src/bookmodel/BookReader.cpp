...
void BookReader::addImage(const std::string &id, shared_ptr<const ZLImage> image) {
  ...
  //  ZDZLImage* zdzlimage = IOSUtil::createZDZLImage(*image);
    //IOSUtil::Method_BookModel_addImage(myModel.myJavaModel,id,*zdzlimage);
    IOSUtil::Method_BookModel_addImage(myModel.myJavaModel,id,(const ZLFileImage&)*image);
    // zhangdong
