...
shared_ptr<Book> Book::loadFromFile(const ZLFile &file) {
	...
	shared_ptr<FormatPlugin> plugin = collection.plugins(file, false);
	if (plugin.isNull()) {
		return 0;
	}
...
*/
 ...
 shared_ptr<Book> Book::loadFromBookPath(const std::string &bookPath)
{
    return createBook(ZLFile(bookPath), 0, "", "", "");
}
