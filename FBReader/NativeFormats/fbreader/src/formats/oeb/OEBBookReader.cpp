...
static const std::string DC_LANGUAGE = "dc:language";
...
void OEBBookReader::startElementHandler(const char *tag, const char **xmlattributes) {
	...
	switch (myState) {
		case READ_NONE:
            if(testOPFTag(DC_LANGUAGE,tagString)){
                //std::string lan =
                myState = READ_LANGUAGE;
            }
            else if (testOPFTag(MANIFEST, tagString)) {
		...
		case READ_MANIFEST:
			if (testOPFTag(ITEM, tagString)) {
				...
				if (href != 0) {
                    ...
                    if(id != 0 && strcmp(id,"titlepage")==0){
                        printf("MMMMMMMM title page\n");
                        break;
                    }
					if (id != 0) {
						myIdToHref[id] = sHref;
...
void OEBBookReader::characterDataHandler(const char *text, std::size_t len)
{
    if(READ_LANGUAGE == myState) {
        std::string s(text,len);
        printf("language is %s\n",s.c_str());
        myModelReader.model().book()->setLanguage(s);
    }
}
